...
public interface OperationComptableRepository extends R2dbcRepository<OperationComptable, UUID> {
        ...
        Flux<OperationComptable> findByTenant_Id(@Param("organization_id") UUID organization_id);

        @Query("SELECT * FROM operation_comptable WHERE organization_id = :organization_id AND operation_id = :id")
        Mono<OperationComptable> findByTenant_IdAndId(@Param("organization_id") UUID organization_id, @Param("id") UUID id);
