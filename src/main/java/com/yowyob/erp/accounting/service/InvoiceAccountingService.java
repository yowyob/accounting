...
public class InvoiceAccountingService {
    ...
    public Mono<Object> accountSupplierInvoice(SupplierInvoiceDto invoice) {
        return ReactiveOrganizationContext.getOrganizationId()
                .flatMap(organization_id -> {
                    log.info("Processing supplier invoice: {} for organization: {}", invoice.getNumeroFacture(), organization_id);
    ...
    public Mono<Object> accountCustomerInvoice(CustomerInvoiceDto invoice) {
        return ReactiveOrganizationContext.getOrganizationId()
                .flatMap(organization_id -> {
                    log.info("Processing customer invoice: {} for organization: {}", invoice.getNumeroFacture(), organization_id);
    ...
    private Mono<com.yowyob.erp.accounting.entity.JournalComptable> findJournal(UUID organization_id, String type) {
        return journal_repository.findByOrganization_IdAndType_journal(organization_id, type)
                ...
                .next()
                .switchIfEmpty(Mono
                        .error(new BusinessException("No active journal of type " + type + " found for this organization")));
    }
