...
public class OrganizationController {
    ...
    private final OrganizationNameEnricher organizationNameEnricher;
    ...
    public Mono<ApiResponseWrapper<OrganizationDto>> getOrganization(
            @PathVariable UUID id,
            @RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authorization,
            @RequestHeader(value = "X-Tenant-Id", required = false) String tenantHeader,
            @RequestHeader(value = "X-Tenant-ID", required = false) String tenantHeaderAlt) {
        String resolvedTenantId = firstNonBlank(tenantHeader, tenantHeaderAlt);
        return organization_service.getOrganization(id)
                .flatMap(dto -> ReactiveOrganizationContext.getTenantId()
                        .map(UUID::toString)
                        .defaultIfEmpty(resolvedTenantId != null ? resolvedTenantId : "")
                        .flatMap(tenantId -> organizationNameEnricher.enrichIfNeeded(
                                dto,
                                authorization,
                                tenantId.isBlank() ? null : tenantId,
                                id.toString())))
