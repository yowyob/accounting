...
public class EcritureComptableController {
        ...
        @Operation(summary = "Cancel an accounting entry")
        public Mono<ResponseEntity<ApiResponseWrapper<EcritureComptableDto>>> cancel(@PathVariable UUID id,
                        @RequestParam(required = false) String user) {
        ...
        @Operation(summary = "Deactivate an accounting entry (soft delete)")
        public Mono<ResponseEntity<ApiResponseWrapper<Object>>> deactivate(@PathVariable UUID id) {
                return ecriture_service.deactivateEcriture(id)
                                .then(Mono.fromCallable(
                                                () -> ResponseEntity.ok(ApiResponseWrapper.success(null,
