...
public class CompteController {
        ...
        /**
         * Generates and creates a Third-Party account (Client/Supplier).
         */
        @PostMapping("/generate")
        @Operation(summary = "Generate and create a Third-Party account")
        public Mono<ResponseEntity<ApiResponseWrapper<CompteDto>>> createAutoGeneratedAccount(
                ...
                if (name == null || type == null) {
                        return Mono.error(new BusinessException("Name and Type are required"));
                }

                log.info("Request to generate {} account for {}", type, name);
                return compte_service.createThirdPartyAccount(name, type, notes)
                                .map(saved -> ResponseEntity.status(HttpStatus.CREATED)
                                                .body(ApiResponseWrapper.success(saved,
                                                                "Third-Party account created successfully")))
                                .contextWrite(ReactiveTenantContext.captureFromThreadLocal());
        }
